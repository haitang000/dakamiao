    ...
    private fun buildSteps(): List<Step> = listOf(
        ...
        Step(
            ...
            desc = "打卡喵是一个自动化辅助工具，随时可能因钉钉更新、界面改版、网络/定位/系统限制等原因失效或点错，不保证每次成功。\n\n" +
                    "• 打卡是否成功，请务必自行到钉钉里核对——不要以为开着就一定打上了。\n" +
                    "• 因漏打卡、打卡失败或异常导致的任何后果（考勤扣款、迟到记录等）由你本人承担，与本软件及作者无关。\n" +
                    "• 本软件的源代码在 GitHub 开源，您可以免费下载，如果您是通过不明渠道下载的本软件，可能会包含恶意代码，我们推荐您立即删除来自该渠道的软件并从本软件的 GitHub 仓库下载。任何因从第三方下载该软件导致的隐私泄露等安全后果，本软件开发者概不负责。\n" +
        ...
        Step(
            emoji = "🖐️",
            title = "① 开启无障碍服务（核心）",
            desc = "这是打卡喵的核心，没有它将无法正常使用打卡喵：靠它读取钉钉界面、模拟点击打卡按钮。不开就无法自动打卡。\n在列表里找到「打卡喵」并打开即可。",
            actionLabel = "去开启无障碍",
